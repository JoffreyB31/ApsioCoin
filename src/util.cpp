...
const char * const BITCOIN_CONF_FILENAME = "apsiocoin.conf";
const char * const BITCOIN_PID_FILENAME = "apsiocoind.pid";
...
#else
    const char* pszModule = "apsiocoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Apsiocoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Apsiocoin";
#else
    // Unix
    return pathRet / ".apsiocoin";
#endif
#endif
}
